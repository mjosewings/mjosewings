## Maria's Lab of Projects

Hello there, I'm **Maria** -- a fourth-year Computer Science student specializing in Data Science, aspiring to become a bioinformatician focused on genomics, eager to tackle challenges in drug discovery using computational tools.

---

## Current and Upcoming Projects 

### Black History Artificial Intelligence (BHAI) - Reimagining Equity, Diversity, and Inclusion Through AI *(Expected to release in 2030)*
Black History Artificial Intelligence (BHAI) presents a conceptual AI platform designed to educate and empower users by highlighting Black history, culture, and linguistic heritage.

### Online Misinformation Machine Learning *(Coming Soon)*

### Data Mining Exploration *(Coming Soon)*

### Mini-Pupper Maze Mission *(Coming Soon)*
Designing and programming a robotic mini-pupper to navigate mazes autonomously, combining robotics, AI logic, and sensor integration.

--

## Skills and Tools

### Programming and Scripting
- **Python** - Data analysis, automation
-  **R** - Statistical analysis, data visualization
-  **Javascript** - Interactive web elements, front-end logic
-  **HTML & CSS** - Styling and structure for web projects

### Databases & SQL
- **MySQL** - CRUD operations and relational database design
- **

---
# Connect with Me
- LinkedIn:[https://www.linkedin.com/in/maria-joseph-a6500a24b/]
- Email: maj5833@psu.edu
- Porfolio: Coming Soon!

--
<!--
**mjosewings/mjosewings** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
